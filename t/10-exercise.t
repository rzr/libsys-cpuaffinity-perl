...
ok($z != 0, "simple setCpuAffinity returned non-zero")
    or diag "simple setCpuAffinity($simpleMask) return value was $z $! $^E";

my $y1 = Sys::CpuAffinity::getAffinity($$) || 0;
ok($y1 == $simpleMask, 
   "setCpuAffinity set affinity to $y1 == $simpleMask != $y");
if ($y1 != $simpleMask && $^O =~ /linux/i) {
    $y1 = Sys::CpuAffinity::_getAffinity_with_xs_DEBUG_sched_getaffinity($$);
}
